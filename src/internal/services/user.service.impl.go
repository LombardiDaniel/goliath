...
func (s *UserServicePgImpl) CreateUnconfirmedUser(ctx context.Context, unconfirmedUser domain.UnconfirmedUser) error {
	...
	if count != 0 {
		return errors.Join(constants.ErrDbConflict, errors.New("user already exists"))
	}
...
func (s *UserServicePgImpl) GetUser(ctx context.Context, email string) (domain.User, error) {
	...
	if err != nil {
		return user, errors.Join(err, validators.FilterSqlPgError(err))
	}
...
func (s *UserServicePgImpl) GetUsers(ctx context.Context) ([]domain.User, error) {
	...
	if err != nil {
		return users, errors.Join(err, validators.FilterSqlPgError(err))
	}
	defer rows.Close()
...
func (s *UserServicePgImpl) GetUserOrgs(ctx context.Context, userId uint32) ([]dto.OrganizationOutput, error) {
	query := `
		SELECT DISTINCT
			o.organization_id,
			o.organization_name,
	...
	orgs := []dto.OrganizationOutput{}

	rows, err := s.db.QueryContext(ctx, query, userId)
	if err != nil {
		return orgs, errors.Join(err, validators.FilterSqlPgError(err))
	}
	defer rows.Close()
...
func (s *UserServicePgImpl) InitPasswordReset(ctx context.Context, userId uint32, otp string) error {
	...
	_, err := s.db.ExecContext(ctx, query,
		userId,
		otp,
		time.Now().Add(24*time.Hour*time.Duration(constants.PasswordResetTimeoutDays)),
	)
...
func (s *UserServicePgImpl) GetPasswordReset(ctx context.Context, otp string) (domain.PasswordReset, error) {
	...
	return passReset, errors.Join(err, validators.FilterSqlPgError(err))
}
...
func (s *UserServicePgImpl) UpdateUserPassword(ctx context.Context, userId uint32, pw string) error {
	...
	pwHash, err := token.HashPassword(pw)
	if err != nil {
		return errors.Join(err, validators.FilterSqlPgError(err))
	}
...
func (s *UserServicePgImpl) EditUser(ctx context.Context, userId uint32, user dto.EditUser) error {
