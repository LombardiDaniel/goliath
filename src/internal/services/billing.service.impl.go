...
func NewBillingService(db *sql.DB, stripeApiKey string) BillingService {
	successUrl, err := url.JoinPath(constants.AppHostUrl, "/billing/success")
	if err != nil {
		panic(err)
	}

	cancelUrl, err := url.JoinPath(constants.AppHostUrl, "/billing/cancel")
	if err != nil {
		panic(err)
	}
...
func (s *BillingServiceStripeImpl) SetCheckoutSessionAsComplete(ctx context.Context, sessionId string) (domain.Payment, error) {
	...
	return p, errors.Join(err, validators.FilterSqlPgError(err))
}
